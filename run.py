...
class GameController(object):
    ...
    def setPacmanInRandomPosition(self):
        if self.pacman is None:
            return
        nodes = self.nodes.nodesLUT.values()

        def filterNodes(x: Node, position, radius=50.0) -> bool:
            distance = (x.position - position).magnitude()
            return distance > radius

        nodes = filter(
            lambda x: filterNodes(x, self.ghosts.blinky.position),
            nodes,
        )
        nodes = filter(
            lambda x: filterNodes(x, self.ghosts.clyde.position),
            nodes,
        )
        nodes = filter(
            lambda x: filterNodes(x, self.ghosts.inky.position),
            nodes,
        )
        nodes = filter(
            lambda x: filterNodes(x, self.ghosts.pinky.position),
            nodes,
        )
        randomPosition = random.choice(list(nodes))
        self.pacman = Pacman(randomPosition)
